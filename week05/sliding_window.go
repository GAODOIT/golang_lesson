...
func (l *SlidingWindowLimiter) Run(fn func()) error {
	if !l.Allow() {
		return errors.New("ErrFreqExceed")
	}
...
func main( ) {
	limter := NewSlidingWindowLimiter(100*time.Millisecond, 1*time.Second, 10) // QPS = 100, windowNum = 10
	...
	handler := func(expectSucc, expectFail int64) {
		...
		if succ != expectSucc || fail != expectFail {
			fmt.Printf("i:%d expect succ:%d fail:%d, actual succ:%d fail:%d\n", i, expectSucc, expectFail, succ, fail)
		}
	}
